...
SIZE_T sys_fread(struct file *fp, void *buf, SIZE_T size, SIZE_T count, SIZE_T offset)
{
    loff_t pos = offset;
    SIZE_T res;
    ...
    res = kernel_read(fp, buf, size * count, &pos);
    if (res > 0) {
        // You can update the file position if needed
        // fp->f_pos = pos;
        // printk("read size:%zu", res);
    }
